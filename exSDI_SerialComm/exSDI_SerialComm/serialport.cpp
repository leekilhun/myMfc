...
bool CSerialPort::WriteByte(BYTE * sendbyte, UINT size)
{
	//iBytesWritten=0;
	//m_iBytesWritten=0;  
	if (WriteFile(m_hComm, sendbyte, size, &m_iBytesWritten, NULL) == 0)
		return false;
	else
		return true;
}
...
bool CSerialPort::ReadByte(BYTE * resp, UINT size)
{
	...
	if (ReadFile(m_hComm, buff, size, &dwBytesTransferred, 0))
	{
