...
// exMfcSerialCommDlg.cpp: 구현 파일
//

#include "pch.h"
#include "framework.h"
#include "exMfcSerialComm.h"
#include "exMfcSerialCommDlg.h"
...
// CexMfcSerialCommDlg 대화 상자



CexMfcSerialCommDlg::CexMfcSerialCommDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_EXMFCSERIALCOMM_DIALOG, pParent)
	, m_str_edit_value(_T(""))
	, m_edit_value(_T(""))
	, m_str_combo(_T(""))
	, radio(0)
...
void CexMfcSerialCommDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
...
BEGIN_MESSAGE_MAP(CexMfcSerialCommDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()

	ON_WM_HSCROLL()
	ON_MESSAGE(WM_MYMESSAGE, &CexMfcSerialCommDlg::OnReceive)


	ON_BN_CLICKED(IDC_BUTTON1, &CexMfcSerialCommDlg::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON2, &CexMfcSerialCommDlg::OnBnClickedButton2)
	ON_NOTIFY(UDN_DELTAPOS, IDC_SPIN3, &CexMfcSerialCommDlg::OnDeltaposSpin3)
	//	ON_EN_CHANGE(IDC_EDIT4, &CexMfcSerialCommDlg::OnEnChangeEdit4)
	ON_NOTIFY(UDN_DELTAPOS, IDC_SPIN4, &CexMfcSerialCommDlg::OnDeltaposSpin4)

	ON_BN_CLICKED(IDC_BUTTON4, &CexMfcSerialCommDlg::OnBnClickedButton4)
	ON_BN_CLICKED(IDC_BUTTON3, &CexMfcSerialCommDlg::OnBnClickedButton3)
	ON_CBN_SELCHANGE(IDC_COMBO2, &CexMfcSerialCommDlg::OnCbnSelchangeCombo2)
	//ON_CONTROL_RANGE(BN_CLICKED, IDC_RADIO1, IDC_RADIO4, RadioCtrl)
	ON_BN_CLICKED(IDC_RADIO1, &CexMfcSerialCommDlg::OnBnClickedRadio1)
	ON_BN_CLICKED(IDC_RADIO2, &CexMfcSerialCommDlg::OnBnClickedRadio2)
	ON_BN_CLICKED(IDC_RADIO3, &CexMfcSerialCommDlg::OnBnClickedRadio3)
	ON_BN_CLICKED(IDC_RADIO4, &CexMfcSerialCommDlg::OnBnClickedRadio4)
	ON_CBN_SELCHANGE(IDC_COMBO1, &CexMfcSerialCommDlg::OnCbnSelchangeCombo1)
	ON_CBN_SELCHANGE(IDC_COMBO3, &CexMfcSerialCommDlg::OnCbnSelchangeCombo3)
	ON_CBN_SELCHANGE(IDC_COMBO4, &CexMfcSerialCommDlg::OnCbnSelchangeCombo4)
	ON_CBN_SELCHANGE(IDC_COMBO5, &CexMfcSerialCommDlg::OnCbnSelchangeCombo5)
END_MESSAGE_MAP()


// CexMfcSerialCommDlg 메시지 처리기

BOOL CexMfcSerialCommDlg::OnInitDialog()
{
...
void CexMfcSerialCommDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CexMfcSerialCommDlg::OnPaint()
{
	if (IsIconic())
	{
...
//  이 함수를 호출합니다.
HCURSOR CexMfcSerialCommDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}


void CexMfcSerialCommDlg::ProcessErrorMessage(CString msg)
{
...
void CexMfcSerialCommDlg::OnBnClickedButton1()
{
...
void CexMfcSerialCommDlg::ClearAll()
{
	CDC* pDC;
	CRect rect;
	CBrush brush;
...
LRESULT CexMfcSerialCommDlg::OnReceive(WPARAM wParam, LPARAM IParam) // serial 통신을 해서 데이터를 받았을 때 처리해주는 부분 
{
	BYTE buffer;
	CString message;
	int length = -1;
...
void CexMfcSerialCommDlg::PutChar(unsigned char ch)
{
	CDC* pDC;
	TEXTMETRIC tm;
...
void CexMfcSerialCommDlg::DeleteLine(int pos, int type)
{
	CDC* pDC;
	TEXTMETRIC tm;
	CRect rect;
...
CString CexMfcSerialCommDlg::TextFileParsing(CString str, INT len)
{
	if (str.IsEmpty())
	{
		return _T("");
...
CString  CexMfcSerialCommDlg::GetMiddleString(CString str, INT len)
{
	//Thread.Sleep(50);
...
void CexMfcSerialCommDlg::OnSerialInit()
{
	if (m_nPort == 0)
	{
...
void CexMfcSerialCommDlg::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	CString strTemp;
...
BOOL CexMfcSerialCommDlg::IsAnsiCommand(unsigned char ch)
{
...
void CexMfcSerialCommDlg::OnBnClickedButton2()
{
	flag = true;
...
void CexMfcSerialCommDlg::OnDeltaposSpin3(NMHDR* pNMHDR, LRESULT* pResult)
{
...
// 상승 버튼 눌었을 경우 1이 증가 감소 버튼을 눌렀을 경우 1이 감소 
// 최소 값 1 최대값 8로 설정 되어져 있음
void CexMfcSerialCommDlg::OnDeltaposSpin4(NMHDR* pNMHDR, LRESULT* pResult)
{
	LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
...
void CexMfcSerialCommDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	if (pScrollBar)
	{
		int id_index = _ttoi(channel_data) - 1;
...
void CexMfcSerialCommDlg::OnBnClickedButton4()
{
...
void CexMfcSerialCommDlg::OnBnClickedButton3()
{
	/*CString data = IDInitailize('0', '1');
	m_ComuPort.WriteComm(data, 9);
...
void CexMfcSerialCommDlg::OnCbnSelchangeCombo2()
{
...
void CexMfcSerialCommDlg::OnBnClickedRadio1()
{
	int id_index = _ttoi(channel_data) - 1;
...
void CexMfcSerialCommDlg::OnBnClickedRadio2()
{
	int id_index = _ttoi(channel_data) - 1;
...
void CexMfcSerialCommDlg::OnBnClickedRadio3()
{
	int id_index = _ttoi(channel_data) - 1;
...
void CexMfcSerialCommDlg::OnBnClickedRadio4()
{
	int id_index = _ttoi(channel_data) - 1;
...
void CexMfcSerialCommDlg::OnCbnSelchangeCombo1()
{
...
void CexMfcSerialCommDlg::OnCbnSelchangeCombo3()
{
...
void CexMfcSerialCommDlg::OnCbnSelchangeCombo4()
{
	int index = _ttoi(channel_data) - 1;
...
void CexMfcSerialCommDlg::OnCbnSelchangeCombo5()
{
	int index = _ttoi(channel_data) - 1;
...
// ui 초기 세팅 부분을 함수로 만듬
void CexMfcSerialCommDlg::setting()
{
...
void CexMfcSerialCommDlg::getSerialPort()
{
